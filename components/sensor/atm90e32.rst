...
- **phase_a** (*Optional*): The configuration options for the 1st phase.
  ...
  - **voltage** (*Optional*): Use the voltage value of this phase in volts :abbr:`RMS (root mean square)`.
    All options from :ref:`Sensor <config-sensor>`.
  - **current** (*Optional*): Use the current value of this phase in amperes :abbr:`RMS (root mean square)`. All options from
  ...
  - **power_factor** (*Optional*): Use the power factor value on this phase. All options from
    :ref:`Sensor <config-sensor>`.
  - **phase_angle** (*Optional*): Use the current phase angle value on this phase in degrees. All options from
    :ref:`Sensor <config-sensor>`.
  - **voltage_phase_angle** (*Optional*): Use the voltage phase angle value on this phase in degrees. Note that some
    devices (like the CircuitSetup 6-Channel board) connect the same voltage reference to all 3 phase inputs of a
    single chip, and on such a device this sensor will always read 0. All options from
    :ref:`Sensor <config-sensor>`.
  - **peak_current** (*Optional*): Use the peak current value on this phase in amperes. All options from
    :ref:`Sensor <config-sensor>`.
  - **fundamental_power** (*Optional*): Use the fundamental power value on this phase. All options from
    :ref:`Sensor <config-sensor>`.
...
- **neutral_current** (*Optional*): Use the :abbr:`RMS (root mean square)` neutral current value calculated by the
  meter. This value is RMS(Ia + Ib + Ic). This register is only meaningful if you are monitoring all the phases of a
  single circuit on the same ATM90E32. All options from :ref:`Sensor <config-sensor>`.
...
Note that the current measurement is the :abbr:`RMS (root mean square)` value so is always positive.  They only way to
determine direction is to look at the power factor.  If there are only largely resistive loads and no power sources,
(PF almost 1), it is simpler to just create a template sensor that computes power from Irms*Vrms and ignore all these
details.  On the other hand, one might be surprised how reactive some loads are and the CirciuitSetup designs are able
